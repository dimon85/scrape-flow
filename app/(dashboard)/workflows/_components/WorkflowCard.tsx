...
const WorkflowCard = ({ workflow }: { workflow: Workflow }) => {
  ...
  return (
    <Card className="border border-separate shadow-sm rounded-lg owerflow-hidden hover:shadow-md dark:shadow-primary/30 group/card">
      <CardContent className="p-4 flex items-center justify-between h-[100px]">
        <div className="flex items-center justify-end space-x-3">
          ...
          <div>
            <h3 className="text-base font-bolid text-muted-foreground flex items-center">
...
function WorkflowActions({ workflowName, workflowId }: { workflowName: string; workflowId: string; }) {
  const [showDeliteDialog, setShowDeliteDialog] = useState(false);
  return (
    <>
      <DeleteWorkflowDialog
        open={showDeliteDialog}
        setOpen={setShowDeliteDialog}
...
function ScheduleSection({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <SchedulerDialog workflowId={workflowId} cron={cron} key={`${cron}_${workflowId}`} />
